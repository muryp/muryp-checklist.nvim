...
function M.cekBottomChekbox(args)
  ...
  if not STORE.LAST_CONTENT then
    ...
    STORE.INDENT_CHAR = string.match(STORE.PARENT_CONTEN, RG.indent) ---@type string
    ...
    local isUnCheckedBottom = string.match(BOTTOM_CONTEN, RG.unchecked)
    ...
    for NEXT_LINE_NUM = CURRENT_LINE_NUM + 1, STORE.TOTAL_LINES do
      ...
      local isChecked = string.match(NEXT_CONTENT_LINE, RG.checklist)
      local isUnChecked = string.match(NEXT_CONTENT_LINE, RG.unchecked)
      local isCheckbox = isChecked or isUnChecked
      local INDENT_NEXT_CHAR = string.match(NEXT_CONTENT_LINE, RG.indent)
      ...
      if STORE.childernCheck and isChildern and isCheckbox or isContentBlank then
        ...
        local INDENT_ROOT_CHAR = string.match(STORE.PARENT_CONTEN, RG.indent) ---@type string
  ...
  if isParentTrue then
    ...
    local PARENT_NEXT_INDENT = string.match(PARENT_NEXT_CONTENT, RG.indent)
    local PARENT_CONTENT = vim.api.nvim_buf_get_lines(0, isParentTrue - 1, isParentTrue, true)[1]
    local PARENT_INDENT = string.match(PARENT_CONTENT, RG.indent)
