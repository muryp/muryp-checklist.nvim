...
describe('Toggle checkbox on ', function()
  ...
  it('line : 43', function()
    vim.api.nvim_win_set_cursor(0, { 43, 0 })
    toggleCheckbox()
    local CONTENT = vim.api.nvim_get_current_line() ---@type string - get text on current line
    _G.test('- non task', CONTENT)
    toggleCheckbox()
    CONTENT = vim.api.nvim_get_current_line() ---@type string - get text on current line
    _G.test('- [ ] non task', CONTENT)
  end)
end)
